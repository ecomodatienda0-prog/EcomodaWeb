...
const convertToDDMMYY = (yyyyMMDD: string): string => {
  if (!yyyyMMDD) return "";
...
export default function AdminDashboard({
  ...
  const [brokenUserPhotos, setBrokenUserPhotos] = useState<Record<string, boolean>>({});
  ...
  const [editedPrefix, setEditedPrefix] = useState(() => {
    if (companyInfo.prefijoPais) return companyInfo.prefijoPais;
    const match = companyInfo.phone ? companyInfo.phone.match(/^(\+\d+)\s*(.*)/) : null;
    return match ? match[1] : "+504";
  });
  const [editedPhoneNoPrefix, setEditedPhoneNoPrefix] = useState(() => {
    const pref = companyInfo.prefijoPais || "+504";
    if (companyInfo.phone && companyInfo.phone.startsWith(pref)) {
      return companyInfo.phone.slice(pref.length).trim();
    }
    const match = companyInfo.phone ? companyInfo.phone.match(/^(\+\d+)\s*(.*)/) : null;
    return match ? match[2] : companyInfo.phone;
  });
  ...
  const [editedDepartment, setEditedDepartment] = useState(companyInfo.department || "");
  const [editedMunicipio, setEditedMunicipio] = useState(companyInfo.municipio || "");
  const [editedDireccionExacta, setEditedDireccionExacta] = useState(companyInfo.direccionExacta || "");
  const [editedGoogleMapsLink, setEditedGoogleMapsLink] = useState(companyInfo.googleMapsLink || "");
  const [editedLogo, setEditedLogo] = useState(companyInfo.logo || "");
  ...
  // States for live camera capturing
  const [uploadMode, setUploadMode] = useState<"file" | "camera">("file");
  const [cameraStream, setCameraStream] = useState<MediaStream | null>(null);
  const [cameraError, setCameraError] = useState<string | null>(null);
  const videoRef = React.useRef<HTMLVideoElement | null>(null);

  // Clean up camera on change of mode or when uploader closes
  useEffect(() => {
    if (!selectedProductForUploader || uploadMode !== "camera") {
      if (cameraStream) {
        cameraStream.getTracks().forEach((track) => track.stop());
        setCameraStream(null);
      }
    }
  }, [selectedProductForUploader, uploadMode]);
  ...
  const startCamera = async () => {
    ...
    try {
      ...
      console.error("Camera access error, retrying simpler options:", err);
      try {
        const stream = await navigator.mediaDevices.getUserMedia({
          video: true,
          audio: false
        ...
        setCameraError("No se pudo acceder a la cámara. Asegúrese de otorgar permisos.");
      }
    }
  };
  ...
  const capturePhoto = async (product: Product) => {
    ...
    try {
      ...
      if (ctx) {
        ...
        const response = await fetch("/api/upload", {
          ...
          body: JSON.stringify({
            filename: `captured-${product.id}-${Date.now()}.jpg`,
            base64: base64String
          })
        });
        ...
        if (data.success && data.imageUrl) {
          ...
          alert("¡Foto capturada y asociada correctamente!");
      ...
      console.error(err);
      alert(`Error al capturar/subir imagen de la cámara: ${err.message || err}`);
    } finally {
  ...
  useEffect(() => {
    ...
    const pref = companyInfo.prefijoPais || (() => {
      const match = companyInfo.phone ? companyInfo.phone.match(/^(\+\d+)\s*(.*)/) : null;
      return match ? match[1] : "+504";
    })();
    const phoneNoPref = (() => {
    ...
    setEditedPrefix(pref);
    setEditedPhoneNoPrefix(phoneNoPref);
    ...
    setEditedDepartment(companyInfo.department || "");
    setEditedMunicipio(companyInfo.municipio || "");
    setEditedDireccionExacta(companyInfo.direccionExacta || "");
    setEditedGoogleMapsLink(companyInfo.googleMapsLink || "");
    setEditedLogo(companyInfo.logo || "");
  }, [companyInfo]);
  ...
  const handleSaveSettings = async (e: React.FormEvent) => {
    ...
    try {
      const combinedPhone = `${editedPrefix} ${editedPhoneNoPrefix}`.trim();
      await onUpdateConfig({
        ...
        companyInfo: {
          ...
          direccionExacta: editedDireccionExacta,
        ...
        <div className={`p-4 border-t border-slate-800 bg-slate-950/40 text-center space-y-3 ${isMobileMenuOpen ? "block" : "hidden md:block"}`}>
          {currentUser && (
            <div className="bg-slate-900 border border-slate-800 rounded-xl p-2.5 text-left flex items-center gap-2.5">
              {currentUser.foto && !brokenUserPhotos[currentUser.idUsuario || currentUser.correo] ? (
                <img 
                  ...
                  onError={() => {
                    setBrokenUserPhotos(prev => ({ ...prev, [currentUser.idUsuario || currentUser.correo]: true }));
                  }}
                />
              ) : (
        ...
        {activeTab === "inventory" && (
          <div className="space-y-4">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
              ...
              <div className="flex gap-1.5 flex-wrap sm:flex-nowrap w-full sm:w-auto">
                <button
                  type="button"
                  onClick={() => document.getElementById("excel-bulk-import-input")?.click()}
                  disabled={importingExcel}
                  className="bg-white border border-slate-200 hover:bg-slate-50 text-slate-700 px-3 py-2 rounded-lg text-xs font-bold transition-all flex items-center gap-1.5 shadow-3xs cursor-pointer disabled:opacity-50 flex-1 sm:flex-none justify-center"
                >
                ...
                <button
                  ...
                  className="bg-white border border-slate-200 hover:bg-slate-50 text-slate-700 px-3 py-2 rounded-lg text-xs font-bold transition-all flex items-center gap-1.5 shadow-3xs cursor-pointer flex-1 sm:flex-none justify-center"
                >
                ...
                <button
                  ...
                  className="bg-emerald-600 hover:bg-emerald-500 text-white px-3.5 py-2 rounded-lg text-xs font-bold transition-all flex items-center gap-1.5 shadow-xs cursor-pointer whitespace-nowrap flex-1 sm:flex-none justify-center"
                >
                  <Plus className="w-3.5 h-3.5" />
                  Agregar Producto
                </button>
            ...
            <div className="bg-white rounded-2xl border border-slate-100 shadow-3xs p-4">
              <div className="flex justify-between items-center mb-4 gap-4">
                <h3 className="font-bold text-slate-900 text-sm">Productos ({products.length})</h3>
                <div className="relative w-64">
                  ...
                  <input
                    ...
                    className="w-full bg-slate-50 border border-slate-200 focus:border-emerald-500 rounded-lg py-1.5 pl-8 pr-3 text-xs focus:outline-hidden transition-all"
                  />
                </div>
              </div>
              ...
              {filteredProducts.length === 0 ? (
                ...
                <div className="space-y-3">
                  ...
                  <div className="hidden md:block overflow-x-auto">
                    <table className="w-full text-left text-xs border-collapse">
                      <thead>
                        <tr className="border-b border-slate-100 text-slate-500 font-bold bg-slate-50 font-mono text-[10.5px]">
                          <th className="py-2.5 px-3">COD</th>
                          <th className="py-2.5 px-3">IMAGEN</th>
                          <th className="py-2.5 px-3">PRENDA</th>
                          <th className="py-2.5 px-3">STOCK</th>
                          <th className="py-2.5 px-3">PRECIOS (REG / CASH)</th>
                          <th className="py-2.5 px-3">VISIBILIDAD</th>
                      ...
                      <tbody className="divide-y divide-slate-100">
                        {filteredProducts.map((p) => (
                          <tr key={p.id} className="hover:bg-slate-50/40 transition-colors">
                            <td className="py-2 px-3 font-mono font-bold text-slate-450">{p.id}</td>
                            <td className="py-2 px-3">
                              <div className="size-9 rounded-lg overflow-hidden bg-slate-105 border border-slate-100">
                                <img
                                  src={p.imagen}
                                  alt={p.nombre}
                                  referrerPolicy="no-referrer"
                            ...
                            <td className="py-2 px-3 text-slate-650 font-semibold font-mono">{p.stock} pzas.</td>
                            <td className="py-2 px-3">
                              <div>
                                <p className="text-slate-400 line-through text-[10px]">L. {p.precioNormal.toLocaleString()}</p>
                                <strong className="text-emerald-700 font-bold text-[11.5px]">L. {p.precioEfectivo.toLocaleString()}</strong>
                              </div>
                            </td>
                            <td className="py-2 px-3">
                              <button
                                onClick={() => onUpdateProductOverride(p.id, "visible", !p.visible)}
                                className={`px-2 py-0.5 rounded-md text-[9px] font-bold uppercase cursor-pointer ${
                                  p.visible 
                            ...
                            <td className="py-2 px-3 text-center">
                              <button
                                ...
                                className="px-2 py-1 bg-slate-900 hover:bg-slate-850 text-white rounded-lg text-[9.5px] font-bold shadow-3xs flex items-center gap-1 mx-auto cursor-pointer"
                              >
                            ...
                            <td className="py-2 px-3 text-center">
                              <div className="flex gap-1 justify-center">
                                <button
                                  ...
                                  className="p-1 px-2 bg-slate-100 hover:bg-slate-200 text-slate-755 rounded-md text-[9.5px] font-semibold flex items-center gap-0.5 cursor-pointer transition-colors"
                                >
                                  <Edit className="w-2.5 h-2.5 text-indigo-600" /> Editar
                                </button>
                                <button
                                  onClick={() => handleDeleteItem("product", p.id)}
                                  className="p-1 px-1.5 bg-rose-50 hover:bg-rose-100 text-rose-700 rounded-md text-[9.5px] font-semibold flex items-center gap-0.5 cursor-pointer transition-colors"
                                >
  ...
  "Datos_Empresa": ["Nombre", "Prefijo_Pais", "Contacto", "Departamento", "Municipio", "Direccion_Exacta", "Coordendas_Link_Google_Maps", "Logo", "Horarios", "Cuentas_Bancarias", "URL_Apps_Script"],
  ...
  try {
    ...
    if (action === "updateRowValues") {
      ...
      for (var i = 1; i < values.length; i++) {
        var rowVal = values[i];
        if (rowVal && keyColumnIndex < rowVal.length) {
          if (String(rowVal[keyColumnIndex]).trim().toLowerCase() === String(keyValue).trim().toLowerCase()) {
            foundRow = i + 1;
            break;
          }
        }
      ...
      if (foundRow !== -1) {
        var numColsNeeded = rowValues.length;
        var maxCols = sheet.getMaxColumns();
        if (maxCols < numColsNeeded) {
          sheet.insertColumnsAfter(maxCols, numColsNeeded - maxCols);
        }
        sheet.getRange(foundRow, 1, 1, numColsNeeded).setValues([rowValues]);
        return ContentService.createTextOutput(JSON.stringify({ success: true }))
                             .setMimeType(ContentService.MimeType.JSON);
      } else {
        sheet.appendRow(rowValues);
    ...
    if (action === "updateRow") {
      ...
      if (foundRow !== -1) {
        var maxCols = sheet.getMaxColumns();
        if (maxCols < targetColumnIndex + 1) {
          sheet.insertColumnsAfter(maxCols, (targetColumnIndex + 1) - maxCols);
        }
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    <div className="col-span-1">
                      <label className="block text-xs font-bold text-slate-705 mb-1.5">Prefijo *</label>
                      <select
                        value={editedPrefix}
                        onChange={(e) => setEditedPrefix(e.target.value)}
                        className="w-full bg-slate-50 border border-slate-200 focus:border-emerald-500 focus:bg-white rounded-xl py-2.5 px-2 text-xs focus:outline-hidden transition-all font-bold text-slate-800 cursor-pointer"
                      >
                    ...
                    <div className="col-span-2">
                      <label className="block text-xs font-bold text-slate-705 mb-1.5">Número de Contacto *</label>
                      <input
                        type="text"
                        required
                        value={editedPhoneNoPrefix}
                        onChange={(e) => setEditedPhoneNoPrefix(e.target.value.replace(/[^0-9-\s]/g, ""))}
                        placeholder="Ej: 9900-8822"
                        className="w-full bg-slate-50 border border-slate-200 focus:border-emerald-500 focus:bg-white rounded-xl py-2.5 px-3.5 text-xs focus:outline-hidden transition-all font-mono font-bold text-slate-850"
                      />
                    </div>
                  </div>
                  ...
                  <div>
                    <label className="block text-xs font-bold text-slate-700 mb-1.5">Dirección Completa de Sucursal (Concatenada)</label>
                    <input
                      type="text"
                      value={editedAddress}
                      onChange={(e) => setEditedAddress(e.target.value)}
                      placeholder="e.g. Comayagua, Calle Principal"
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      <label className="block text-xs font-bold text-slate-700 mb-1.5">Municipio</label>
                      <input
                        type="text"
                        value={editedMunicipio}
                        onChange={(e) => setEditedMunicipio(e.target.value)}
                        placeholder="e.g. Comayagua"
                        className="w-full bg-slate-50 border border-slate-200 focus:border-emerald-500 focus:bg-white rounded-xl py-2.5 px-3.5 text-xs focus:outline-hidden transition-all text-slate-650"
                      />
                    </div>
                  ...
                  <div>
                    <label className="block text-xs font-bold text-slate-700 mb-1.5">Dirección Exacta</label>
                    <input
                      type="text"
                      value={editedDireccionExacta}
                      onChange={(e) => setEditedDireccionExacta(e.target.value)}
                      placeholder="e.g. Calle Principal contiguo a estadio Carlos Miranda"
                      className="w-full bg-slate-50 border border-slate-200 focus:border-emerald-500 focus:bg-white rounded-xl py-2.5 px-3.5 text-xs focus:outline-hidden transition-all text-slate-650"
                    />
                  </div>

                  <div>
                    <label className="block text-xs font-bold text-slate-700 mb-1.5">Enlace de Coordenadas de Google Maps</label>
                    <input
                      type="text"
                      value={editedGoogleMapsLink}
                      onChange={(e) => setEditedGoogleMapsLink(e.target.value)}
                      placeholder="e.g. https://maps.app.goo.gl/..."
                      className="w-full bg-slate-50 border border-slate-200 focus:border-emerald-500 focus:bg-white rounded-xl py-2.5 px-3.5 text-xs focus:outline-hidden transition-all text-slate-650 font-mono text-[11px]"
                    />
                  </div>

                  <div>
                    <label className="block text-xs font-bold text-slate-700 mb-1.5">URL de la Imagen del Logo</label>
                    <input
                      type="text"
                      value={editedLogo}
                          ...
                          return (
                            <tr key={u.idUsuario} className="hover:bg-slate-50/50 transition-colors">
                              ...
                              <td className="py-3 px-4">
                                {u.foto && !brokenUserPhotos[u.idUsuario] ? (
                                  <img 
                                    referrerPolicy="no-referrer" 
                                    src={u.foto} 
                                    alt={u.nombre} 
                                    ...
                                    onError={() => {
                                      setBrokenUserPhotos(prev => ({ ...prev, [u.idUsuario]: true }));
                                    }}
                                  />
                                ) : (
      ...
      {selectedProductForUploader && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="bg-white w-full max-w-md rounded-2xl shadow-xl overflow-hidden relative z-10 p-6 space-y-4 border border-slate-100">
            ...
            <div className="border border-slate-200 rounded-xl overflow-hidden min-h-[220px] text-center flex flex-col justify-center items-center relative bg-slate-50/50">
              {uploadingProductId === selectedProductForUploader.id ? (
                <div className="space-y-2 py-4">
                  ...
                  <p className="text-xs text-slate-500 font-bold">Procesando y subiendo imagen...</p>
                </div>
              ) : uploadMode === "file" ? (
                <div className="p-8 w-full h-full flex flex-col items-center justify-center gap-2 relative hover:bg-slate-50 transition-colors">
                ...
                <div className="w-full h-full min-h-[220px] flex flex-col items-center relative gap-2 bg-black justify-center overflow-hidden">
                  {cameraError ? (
                    <div className="p-4 text-center space-y-2">
                      ...
                      <button
                        type="button"
                        onClick={startCamera}
                        className="py-1 px-3 bg-white text-slate-900 rounded-lg text-[10px] font-bold"
                      >
                        Reintentar
                      </button>
                    </div>
                    ...
                    <>
                      ...
                      <div className="absolute bottom-2.5 left-1/2 -translate-x-1/2 z-10 flex gap-2">
                        <button
                          type="button"
                          onClick={() => capturePhoto(selectedProductForUploader)}
                          className="bg-emerald-600 hover:bg-emerald-500 text-white font-extrabold text-[11px] px-3.5 py-1.5 rounded-full flex items-center gap-1.5 shadow-md cursor-pointer transition-transform active:scale-95"
                        >
                          <Camera className="w-3.5 h-3.5" />
                          Capturar Foto
                        </button>
                        <button
                          type="button"
                          onClick={startCamera}
                          className="bg-slate-800/80 hover:bg-slate-700 text-white font-extrabold text-[10px] px-2 py-1.5 rounded-full flex items-center gap-1 cursor-pointer"
                        >
            ...
            <form onSubmit={handleModalSubmit} className="space-y-4 text-xs">
              {activeModal === "product" && (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-1">
                    <label className="block font-bold text-slate-750">Vencimiento del Descuento (DD-MM-YY) *</label>
                    <input
                      type="date"
                      value={convertToYYYYMMDD(formData.fechaHasta)}
                      onChange={(e) => setFormData({ ...formData, fechaHasta: convertToDDMMYY(e.target.value) })}
                      onKeyDown={(e) => e.preventDefault()}
                      className="w-full bg-slate-50 border border-slate-200 focus:border-emerald-500 focus:bg-white rounded-xl py-2 px-3 text-xs focus:outline-hidden text-slate-905 font-bold cursor-pointer"
                    />
                  </div>
